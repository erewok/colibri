...
//! Example usage:
//! ```rust
//! use colibri::gossip::messages::{GossipPacket, GossipMessage, ClusterMembership};
//!
//! # fn example() -> Result<(), Box<dyn std::error::Error>> {
//! // Create a sample gossip packet
//! let membership = ClusterMembership::new(1, "127.0.0.1:8001".parse()?);
//! let gossip_packet = GossipPacket::new(GossipMessage::MembershipUpdate {
//!     membership,
//!     sender_node_id: 1,
//! });
//!
//! // For internal gossip (compact, fast)
//! let bytes = bincode::serialize(&gossip_packet)?;
//! let packet = bincode::deserialize::<GossipPacket>(&bytes)?;
//!
//! // For external APIs (readable, standard)
//! let json = serde_json::to_string(&gossip_packet)?;
//! let packet = serde_json::from_str::<GossipPacket>(&json)?;
//! # Ok(())
//! # }
//! ```
...
///
/// Uses serde traits for both INTERNAL (bincode) and EXTERNAL (serde_json) serialization.
/// Choose serialization format at call site based on communication context.
...
pub struct ClusterMembership {
    ...
    pub consistent_hash_buckets: u32,
...
impl GossipPacket {
    ...
    /// Optional: Serialize for EXTERNAL debugging/logging (development only)
    ///
    /// Uses serde_json for human-readable format. This should ONLY be used
    /// for debugging, logging, or external API responses - never for cluster communication.
    #[cfg(debug_assertions)]
    pub fn serialize_json(&self) -> Result<String, serde_json::Error> {
        serde_json::to_string_pretty(self)
    }

    /// Optional: Deserialize from JSON for testing/debugging
    #[cfg(debug_assertions)]
...
impl ClusterMembership {
    ...
    pub fn new(local_node_id: u32, local_address: std::net::SocketAddr) -> Self {
        ...
        Self {
            nodes,
            consistent_hash_buckets: 1, // Start with one node
    ...
    pub fn add_node(&mut self, node_info: NodeInfo) -> bool {
        ...
        self.consistent_hash_buckets = self.nodes.len() as u32;
    ...
    /// Determine which node should own a specific key using consistent hashing
    pub fn get_key_owner(&self, key: &str) -> Option<u32> {
        if self.consistent_hash_buckets == 0 {
            return None;
        }

        // Use consistent hashing to determine ownership
        let bucket =
            crate::consistent_hashing::jump_consistent_hash(key, self.consistent_hash_buckets);

        // Find the active node for this bucket
        let active_nodes: Vec<u32> = self
            .nodes
            .iter()
        ...
        // Map bucket to actual node ID
        active_nodes
            .get(bucket as usize % active_nodes.len())
            .copied()
    }

    /// Get all keys that should be owned by a specific node
    pub fn get_keys_for_node(&self, node_id: u32, all_keys: &[String]) -> Vec<String> {
        all_keys
            .iter()
            .filter(|key| self.get_key_owner(key) == Some(node_id))
            .cloned()
            .collect()
    }
...
mod tests {
    ...
    #[test]
    #[cfg(debug_assertions)]
    fn test_gossip_packet_json_serialization() {
        let message = GossipMessage::Heartbeat {
            node_id: 1,
            timestamp: 1234567890,
            membership_version: 5,
        ...
        // Test JSON serialization (for debugging only)
        let json = packet
            .serialize_json()
        ...
        let deserialized =
            GossipPacket::deserialize_json(&json).expect("Failed to deserialize from JSON");
        ...
        match deserialized.message {
            ...
            _ => panic!("Wrong message type after JSON deserialization"),
        }
    }
    ...
    fn test_cluster_membership_creation() {
        ...
        assert_eq!(membership.consistent_hash_buckets, 1);
    ...
    fn test_cluster_membership_add_remove_node() {
        ...
        assert_eq!(membership.consistent_hash_buckets, 2);
    ...
    #[test]
    fn test_cluster_membership_key_ownership() {
        let addr1 = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 7946);
        let addr2 = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 7947);
        let mut membership = ClusterMembership::new(1, addr1);

        // With single node, it should own all keys
        let owner = membership.get_key_owner("test_key");
        ...
        // Now keys should be distributed between nodes
        let owner1 = membership.get_key_owner("key1");
        let owner2 = membership.get_key_owner("key2");

        // Both should return valid node IDs
        assert!(owner1.is_some());
        assert!(owner2.is_some());

        // The same key should always map to the same owner
        assert_eq!(membership.get_key_owner("key1"), owner1);
        assert_eq!(membership.get_key_owner("key2"), owner2);
    }

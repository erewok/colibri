...
impl fmt::Display for SerializationError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            SerializationError::Binary(err) => write!(f, "Binary: {}", err),
        }
    }
}
...
impl std::error::Error for ColibriError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            ...
            ColibriError::Serialization(SerializationError::Binary(err)) => Some(err),
            _ => None,
        }
    }
}
...
impl From<bincode::Error> for ColibriError {
    fn from(err: bincode::Error) -> Self {
        ColibriError::Serialization(SerializationError::Binary(err))
    }
}
...
impl From<anyhow::Error> for ColibriError {
    fn from(err: anyhow::Error) -> Self {
        ...
        } else if err_str.contains("serialize") || err_str.contains("deserialize") {
            ColibriError::Serialization(SerializationError::Binary(bincode::Error::new(
                bincode::ErrorKind::Custom(err_str),
            )))
        } else {

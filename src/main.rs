...
async fn fire_expire(port: u16) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _resp = client.post(format!("http://localhost:{}/expire-keys", port))
        .send()
        .await.map_err(|err| {
            event!(Level::ERROR, message = "Failed to hit expire-keys endpoint", err=format!("{:?}", err));
            err
        })?;
    Ok(())
}
...
async fn main() -> anyhow::Result<()> {
    ...
    // Start server
    info!("Starting Cache Expiry task");
